...
Generated by F</proc/meminfo>.
    ...
    hightotal       -  Total size of the highmem area.
    highfree        -  Total size of memory if free of the highmem area.
    lowtotal        -  Total size of the lowmem area.
    lowfree         -  Total size of memory if free of the lowmem area.
...
sub get {
    ...
    while (my $line = <$fh>) {
        next unless $line =~ /^
            (
                (?:Mem|Swap|High|Low)(?:Total|Free)|
                Buffers|Cached|SwapCached|Active|Inactive|
                Dirty|Writeback|Mapped|Slab|Commit(?:Limit|ted_AS)
            ):\s*(\d+)
        /x;
        my ($n, $v) = ($1, $2);
        $n =~ tr/A-Z/a-z/;
